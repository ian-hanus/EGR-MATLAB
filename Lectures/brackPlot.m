...
%
% Written by Gerald Recktenwald, in
% "Numerical Methods with Matlab"
%
% Edited by Michael R. Gustafson II to exclude roots, 
%            change comment lengths, and
%            add ability to label with function handles
%
...
%            nx = (optional) number of samples along x axis used to test 
%                 for brackets.  The interval xmin <= x <= xmax is 
...
%                 bracket for the k^th potential root.  
%                 If no brackets are found, Xb = [].
...
c = [.9 .4 0];                       %  RGB color used to fill the box

% --- Begin search for brackets
x = linspace(xmin,xmax,nx);    %  Vec. of potential bracket limits
f = feval(fun,x);              %  Vec. of f(x) vals at potential brackets
nb = 0;  Xb = [];              %  Xb is null unless brackets are found
clf
for k = 1:length(f)-1
  if sign(f(k))*sign(f(k+1))==-1  %  True if sign of f(x) changes in interval
    nb = nb + 1;
    Xb(nb,:) = [x(k) x(k+1)];  %  Save left and right ends of the bracket
    hold on;  fill([x(k) x(k) x(k+1) x(k+1) x(k)],ybox,c); %  Add box
  end
end
...
% --- Add plot here so that curve is on top of boxes indicating brackets
plot(xp,yp,[xmin xmax],[0 0]);
grid on;  xlabel('x');
if isa(fun,'inline')
  ylabel(sprintf('Roots of f(x) = %s',formula(fun)));  %  label is f(x)
elseif isa(fun, 'function_handle')
  ylabel(sprintf('Roots of f = %s',func2str(fun)));  %  label is f. handle
else
  ylabel(sprintf('Roots of f(x) defined in %s',fun));  %  label is m-file
end
hold off
