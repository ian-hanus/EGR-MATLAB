...
for k = 1:N
    if k == N
        dydt2F(k) = (y(k) - y(k-1))/dt;
    else
        dydt2F(k) = (y(k+1) - y(k))/dt;
    end
end
...
for k = 1:N 
    if k == 1
        ...
        dydt2B(k) = (y(k) - y(k-1))/dt;
    end
end

for k = 1:N
    if k == 1
        dydt3C(k) = (-1*y(3)+4*y(2)-3*y(1))/(2*dt);
    elseif k==N
        dydt3C(k) = (3*y(k)-4*y(k-1)+y(k-2))/(2*dt);
    else
        dydt3C(k) = (y(k+1)-y(k-1))/(2*dt);
    end
end

for k = 1:N
    if k == 1
        d2ydt23C(k) = (y(3) - 2*y(2) + y(1))/(dt^2);
    elseif k == N
        d2ydt23C(k) = (y(N) - 2*y(N-1) + y(N-2))/(dt^2);
    else 
        d2ydt23C(k) = (y(k+1) - 2*y(k) + y(k-1))/(dt^2);
    end
end
for k = 1:N
if k == 1
    trapint(k) = 0;
elseif k == 2
    trapint(k) = dt/2*(y(1)+y(2));
elseif k == 3
    trapint(k) = dt/2*(y(1)+2*y(2)+y(3));
else 
    trapint(k) = dt/2*(y(1)+2*(sum(y(2:(k-1))))+y(k));
end
end
% Simpson's Hint
for k=1:N
     ...
     elseif k/2 ~= floor(k/2)% replace 1==1 with code to see if k is odd
